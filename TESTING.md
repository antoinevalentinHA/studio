...
| The same for the FS format — the one that goes onto the card | `FsPackRoundTripTest` | `core` module; specifications and characterization together. Assets are made with the project's own converters, since the writer refuses anything but a 320×240 RLE bitmap and a mono MP3. The `KNOWN GAP` cases are the pack uuid (the writer names the folder by its last eight hex digits and the reader takes the folder name), the node uuids (no room in the 44-byte record; invented on read) and a missing audio (a silent placeholder comes back). Also pins the on-disk layout verified against a device in `FORMATS.md` §5–6 |
